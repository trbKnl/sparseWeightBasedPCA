...
#'
#' @param vars A vector with integers indicating the number of variables per data block. For example, c(10, 20, 5), the first 10 variables belong the first block, the next 20 variables belong to the second block, the next 5 variables belong to the third block
...
#' @param filterZeroSegments A boolean with default \code{TRUE}. If set to \code{TRUE} components where all blocks are set to zero are not included. 
#' @return 
