...
* `makeDat` to simulate the data. Check the package documentation for more details.
...
We will now demonstrate data analysis using `mmsca`. In this example we will use the same data as in the `scads` example. We will demonstrate the use of `mmscaHyperCubeSelection` as already for this toy-example tuning an exhaustive grid with `mmscaModelSelection` takes too long. Here we also use cross validation with the one standard error rule.
...
The data generating structure has been estimated back well. You can see some small coefficients still being estimated, if you increase the `grouplasso` these small coefficients will be put to zero. 
